...
public class BasicRobot implements Robot {
	...
	public void rotate(int degree) throws UnsupportedArgumentException {
		...
		Globals.energyConsumed += costRotate;
		batteryCharge -= costRotate;
		PlayActivity.mProgress.incrementProgressBy(-costRotate);
	}
		...
		while(distance > 0) {
			...
			if(forward) {	
				if(checkMove(currDeg, robPos[0], robPos[1])) {
					...
					maze.walk(1);
					PlayActivity.mProgress.incrementProgressBy(-costMove);
					Globals.energyConsumed += costMove;
	...
	public boolean canSeeGoalAhead() throws UnsupportedMethodException {
		...
		PlayActivity.mProgress.incrementProgressBy(-costDistSensing);
		if(!frontSensor)
	...
	public int distanceToObstacleAhead() throws UnsupportedMethodException {
		...
		batteryCharge -= costDistSensing;
		PlayActivity.mProgress.incrementProgressBy(-costDistSensing);
		Globals.energyConsumed += costDistSensing;
		if(!frontSensor)
			throw new UnsupportedMethodException();
