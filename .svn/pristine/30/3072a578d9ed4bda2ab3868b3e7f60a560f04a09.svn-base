...
public class MazeBuilder implements Runnable {
	...
	/**
	 * This method generates pathways into the maze. uses an iterative method in order to come up with a full pathway until 
	 * 
	 */
	protected void generatePathways() {
		int[][] origdirs = new int[width][height] ; 
		
		//get a random starting point along the top of the maze and save it
		...
		int dir = 0; 	 	
		int origdir = dir; 	
		cells.setVisitedFlagToZero(x, y); // visit the first cell
		while (true) { 		
			...
			// if a move cant be made, change direction, else, make the move
			if (!cells.canGo(x, y, dx, dy)) { 
				
				dir = (dir+1) & 3; // rotate direction
				if (origdir == dir) { 
					
					if (x == firstx && y == firsty) // if we are back at the first spot w/ no more valid moves
						break; 
					
					// get numbers to go back as there was no solution through this random instantiation
				...
				// move position to the new square
				x += dx;
				y += dy;
				
				cells.setVisitedFlagToZero(x, y);
				
				// add direction moved to array of directions
				origdirs[x][y] = dir;
				origdir = dir = random.nextIntWithinInterval(0, 3);
			}
		}
	...
	/**
	 * @returns the number of rooms expected
	 */
